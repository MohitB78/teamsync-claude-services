#!/bin/bash
# =============================================================================
# Seed Sample Documents for Admin User - Railway Deployment
# =============================================================================
# This script creates sample documents in MongoDB and uploads files to MinIO
# for the admin@accessarc.com user's personal drive on Railway.
#
# Prerequisites:
...
#
# Usage: ./seed-sample-documents-railway.sh
# =============================================================================
...
echo -e "${GREEN}=== Seeding Sample Documents for Admin User (Railway) ===${NC}"

# =============================================================================
# Railway Configuration (fetched from Railway variables)
# =============================================================================

# MongoDB Configuration
MONGO_PUBLIC_URL="mongodb://mongo:bImjOQVrydytOZWNladJECNlcsJPShJd@yamanote.proxy.rlwy.net:44087"
# IMPORTANT: Content service uses 'teamsync' database (NOT 'teamsync_content')
MONGO_DB_CONTENT="teamsync"
MONGO_DB_PERMISSIONS="teamsync"
...
if ! command -v mc &> /dev/null; then
    ...
    echo -e "${YELLOW}Install with: brew install minio/stable/mc${NC}"
    exit 1
fi
...
# Get Admin User ID
# =============================================================================
echo -e "\n${BLUE}Finding admin user...${NC}"

# Try to get user ID from Keycloak users in MongoDB (AccessArc stores synced users)
ADMIN_USER_ID=$(mongosh "$MONGO_PUBLIC_URL/accessarc?authSource=admin" --quiet --eval "
const user = db.users.findOne({email: '$ADMIN_EMAIL'});
if (user) {
    print(user._id.toString ? user._id.toString() : user._id);
} else {
    print('');
}
...
if [ -z "$ADMIN_USER_ID" ] || [ "$ADMIN_USER_ID" = "null" ]; then
    echo -e "${YELLOW}Admin user not found in AccessArc database.${NC}"
    # Use the default Keycloak admin UUID
    ADMIN_USER_ID="2fd9b5d2-262d-4748-b24f-f8fe544eabad"
    echo -e "${YELLOW}Using default Keycloak admin UUID: $ADMIN_USER_ID${NC}"
fi
...
# Create Sample Files
# =============================================================================
echo -e "\n${BLUE}Creating sample files...${NC}"

# 1. Sample PDF - Company Report
...
# Remove existing alias if exists
mc alias remove "$MINIO_ALIAS" 2>/dev/null || true

# Add Railway MinIO alias
...
mc cp "$TEMP_DIR/Company-Report-2024.pdf" "$MINIO_ALIAS/$BUCKET_NAME/$STORAGE_KEY1"
mc cp "$TEMP_DIR/Project-Proposal.pdf" "$MINIO_ALIAS/$BUCKET_NAME/$STORAGE_KEY2"
mc cp "$TEMP_DIR/Meeting-Notes-Q4.docx" "$MINIO_ALIAS/$BUCKET_NAME/$STORAGE_KEY3"
mc cp "$TEMP_DIR/Annual-Budget-2024.xlsx" "$MINIO_ALIAS/$BUCKET_NAME/$STORAGE_KEY4"
mc cp "$TEMP_DIR/README.txt" "$MINIO_ALIAS/$BUCKET_NAME/$STORAGE_KEY5"
...
echo -e "${GREEN}✓ File sizes: PDF1=$SIZE1, PDF2=$SIZE2, DOCX=$SIZE3, XLSX=$SIZE4, TXT=$SIZE5${NC}"

# =============================================================================
# Create Drive in Permissions Database
# =============================================================================
echo -e "\n${BLUE}Creating drive in permissions database...${NC}"

mongosh "$MONGO_PUBLIC_URL/$MONGO_DB_PERMISSIONS?authSource=admin" --quiet --eval "
...
echo -e "${GREEN}✓ Drive created/verified in permissions database${NC}"

# =============================================================================
# Create Documents in Content Database
# =============================================================================
echo -e "\n${BLUE}Creating documents in content database...${NC}"

mongosh "$MONGO_PUBLIC_URL/$MONGO_DB_CONTENT?authSource=admin" --quiet --eval "
const tenantId = '$TENANT_ID';
const driveId = '$DRIVE_ID';
const ownerId = '$ADMIN_USER_ID';
const now = new Date();
...
// IMPORTANT: entityVersion is required for Spring Data MongoDB @Version annotation
// Without it, Spring thinks the document is new and tries to INSERT instead of UPDATE
const doc1 = {
    _id: '$DOC1_ID',
    entityVersion: NumberLong(0),
    tenantId: tenantId,
...
echo -e "${GREEN}✓ Documents created in content database${NC}"

# =============================================================================
# Update Drive Usage
# =============================================================================
echo -e "\n${BLUE}Updating drive storage usage...${NC}"

TOTAL_SIZE=$((SIZE1 + SIZE2 + SIZE3 + SIZE4 + SIZE5))
...
echo -e "${GREEN}✓ Drive storage usage updated${NC}"
...
mc ls --recursive "$MINIO_ALIAS/$BUCKET_NAME/$TENANT_ID/$DRIVE_ID/" 2>/dev/null || echo "No files found"
...
echo -e "\n${GREEN}=== Done! ===${NC}"
echo -e "Sample documents have been created for admin@accessarc.com on Railway"
echo -e "Personal Drive ID: ${BLUE}$DRIVE_ID${NC}"
echo -e "\nDocuments created:"
echo -e "  1. Company-Report-2024.pdf (PDF) - starred"
echo -e "  2. Project-Proposal.pdf (PDF) - pinned"
echo -e "  3. Meeting-Notes-Q4.docx (Word)"
echo -e "  4. Annual-Budget-2024.xlsx (Excel) - starred"
echo -e "  5. README.txt (Text)"
echo -e "\nTotal storage used: ${BLUE}$TOTAL_SIZE bytes${NC}"
